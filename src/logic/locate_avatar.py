...
from src.logic.locate_avatar_fallback import locate_avatar_fallback
...
def locate_avatar(action: Action) -> None:
    ...
    if max_val < imgconfig.TEMPLATE_MATCH_THRESHOLD:
        # If we already have a position (e.g. from a previous fallback in the same
        # deploy action), reuse it instead of running the expensive click+OCR again.
        if action.avatar_pos is not None:
            logger.warning(
                f"Could not find a good matching avatar for {action.oper}, with max_val: {max_val}; "
                f"reusing previous position {action.avatar_pos}"
            )
            return

        logger.warning(
            f"Could not find a good matching avatar for {action.oper}, with max_val: {max_val}; trying fallback"
        )
        try:
            locate_avatar_fallback(action)
            return
        except ErrorToLog:
            logger.error(f"Fallback also failed to locate {action.oper}")
