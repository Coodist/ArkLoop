...
with open(ROOT / 'resource/map/overview.json', 'r', encoding='utf-8') as file:
    overview_data = json.load(file)

code_to_filename = {value['code']: value['filename'] for value in overview_data.values()}

with open(ROOT / 'resource/level_code_mapping.json', 'w', encoding='utf-8') as file:
...
with open(ROOT / 'resource/level_name_mapping.json', 'w', encoding='utf-8') as file:
